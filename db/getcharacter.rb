...
class GetCharacter
  ...
  def self.allegiances?(id = 708)
    allegiance_array = []
    character_hash = self.spouse?(id)
    character_hash.each do |attr, value|
      if attr == :allegiances && value.first == ""
  ...
  def self.build_character_hash(id = 708)
...
# GetCharacter.build_character_hash
# binding.pry
# p 'eof'
